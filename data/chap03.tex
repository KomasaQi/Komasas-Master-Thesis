...
本节介绍了一种基于图结构的流匹配扩散方法用于对周车进行长时域非确定预测，本节首先从图结构与图神经网络的介绍开始，随后展开了交通图特征表示方法及自回归图扩散交互式预测模型的构建方法。
  ...
  \subsection{流匹配扩散方法}
      ...
      FM用神经网络学习一个任务向量空间上的向量流场：$v_t^\theta$，其损失函数定义为
      \begin{equation}
      \mathcal{L}(\theta) = \mathbb{E_{t,p_t(x)}} \lVert v_t^\theta(x_t) - v_t^{target}(x_t) \rVert^2
      \label{eq:fm_loss_original}
      \end{equation}

      其中$\theta$表示神经网络的参数，$t \in [0,1]$表示时间变量，$x_t$表示在时间$t$时刻的样本，$p_t(x)$表示在时间$t$时刻的样本分布，$v_t^{target}(x_t)$表示在时间$t$时刻的目标速度向量场。
  ...
  \subsection{自回归图扩散交互式预测模型}
    ...
    \subsubsection*{(2)特征提取器设计}
      图编码器采用多层图神经网络结构，对输入的图结构路网进行特征提取与局部、全局信息聚合，并加入时间编码与动作编码引导生成，通过邻居节点与边的特征传递与更新局部特征，通过全局线性注意力捕捉交通场景中的时空依赖关系与交互作用，生成下一时刻$k+1$的节点动态特征变化速度向量集$v^\theta(F_{dyn}^{k+1})$以实现图扩散（$\theta$为Graphflow模型参数）。
      ...
      % 特征提取器输出与FCN的连接设计
      最后，将特征提取器输出连接到多个全连接网络（Fully Connected Network, FCN）并对需要限制范围的输出如$c_{ego}$,$c_{risk}$等加入$\text{Softmax(\cdot)}$函数进行非线性映射。最终将所有输出拼接起来作为模型的最终输出$v^\theta(F_{dyn}^{k+1})$，即第$k+1$时刻所有节点动态节点特征的速度向量。

    \subsubsection*{(3)图扩散流匹配训练}
      流匹配扩散模块基于流匹配理论，学习交通状态的速度向量场。通过最小化条件流匹配（Conditional Flow Matching, CFM）损失函数$\mathcal{L}_{CFM}(\theta)$，训练模型生成符合目标分布的未来交通状态样本。该模块能够有效捕捉交通状态的动态变化，实现对未来交通状态的多模态预测。
      ...
      其中$\tau_{SSCL}$为预设的衰减系数，以$\epsilon_{SSCL}$的概率保留数据集中未来时刻的动态特征真值$F_{dyn}^{k+1}$，以$1-\epsilon_{SSCL}$的概率使用模型预测结果$F_{dyn}^{\theta,k+1}$替换对应真值动态特征在下次自回归过程中共同组成时空图输入。随着训练的进行，$\epsilon_{SSCL}$的值逐渐从1降低至0，使模型逐步适应自回归预测环境，实现了自回归训练稳定性的显著增加。
      ...
      \begin{algorithm}
        ...
        \begin{algorithmic}
          ...
          \ENSURE 给定时刻$t$下图节点所有动态特征变化速度预测$v_t^{\theta}(F_{dyn}^{t}|\mathcal{V}_{k+1}), t\in [k,k+1]$
          ...
          \FOR{$epoch = 1,2,\dots,N_{max,epoch}$}
            \FOR {$i = 1,2,\dots,N_{data}$}
              ...
              \FOR{$k = 0,1,2,\dots,T-1$}
                ...
                \STATE $v_t^{\theta}(F_{dyn}^{t}|\mathcal{V}_{k+1}) \leftarrow \text{GraphflowModel}(\mathcal{G}_{st},t)$
                \STATE $\mathcal{L}_{CFM}(\theta) = \lVert v_t^{\theta}(F_{dyn}^{t}|\mathcal{V}_{k+1}) - v_t^{target}(F_{dyn}^{t}|\mathcal{V}_{k+1}) \rVert^2$
                \STATE 更新参数：$\theta \leftarrow \theta - \eta \nabla_{\theta} \mathcal{L}_{CFM}(\theta)$
                \STATE $F_{dyn}^{\theta,k+1} = F_{dyn}^{k} + \text{Euler}(\text{GraphflowModel}(\mathcal{G}_{st},t))$
                \STATE $\epsilon_{SSCL} = \text{calculateSSCLThred}(N_{batch}) $ 调度采样课程学习
                \IF {$\text{rand}() \leq \epsilon_{SSCL}$} 
                  \STATE $F_{dyn}^{k+1} \leftarrow F_{dyn}^{\theta,k+1}$  以$\epsilon_{SSCL}$的概率保留数据集中未来时刻的动态特征真值$F_{dyn}^{k+1}$
                \ENDIF
    ...
    关于Graphflow方法进行图扩散流匹配理论有效性的证明请参见附录\label{sub:graphflow_proof}节。
...
\section{强化学习长时域决策模型}
  ...
  \label{subsection:param_and_int_recog}
    ...
    \end{cases}
    \end{equation}
    其中$\alpha_L,\beta_L,\alpha_R,\beta_R,p$共5个参数为待识别的模型参数，公用的参数$p$是对应MOBIL模型的礼貌系数，反映这辆车对自车影响其他车的重视程度。
    ...
    采用贝叶斯估计中的MCMC（马尔科夫链蒙特卡罗）方法进行采样对驾驶员模型参数、换道模型参数及意图的不确定性进行更新。如图~\ref{fig:mcmc}所示，MCMC方法构建一个马尔科夫链，从后验分布中抽样出一系列的$\theta$构建参数的概率分布。通过采样得到的$\theta_1,\theta_2,\dots,\theta_{M'}$，可对参数的分布进行估计。如果采样得到了$T_{hw}$的样本值$T_{hw}^{(1)},T_{hw}^{(2)},\dots,T_{hw}^{(M)}$，可以用样本均值作为参数的估计值，用样本方差作为该参数的不确定性，余下的问题在于后验概率如何求得。

    驾驶员模型参数和意图参数识别采用时间触发机制，具体而言采用的意识识别算法为基于规则的可能性识别与贝叶斯推断的结合，通过地图信息与交通规则确定车辆在未来给定的较长一段距离内的意图分叉点（识别出了$K$种可能的意图，比如当前正在高速上行驶，前方500m有匝道口，那么可能的意图包括直行和下匝道，如图~\ref{fig:param_int_uncertain}所示）；并以识别好的驾驶员模型为基础，分别设置车辆为不同的意图进行仿真（每种意图有各自的先验信息，可以通过道路结构、车辆位置进行合理推断给出，比如在匝道口可以根据往常交通流量+当前该车的车道位置+当前该车的加速趋势给出$P(\text{Go Straight})=0.25$，$P(\text{Exit})=0.75)$），利用过去一段时间给定时间窗口内的周车历史数据作为输入进行仿真，获得该车不同意图假设下的行为预测，损失函数采用预测测量与实际观测测量间的L2损失：
    \begin{equation}
    ...
    某组参数和意图的后验概率$\pi(\theta,I)$:各采样时刻（$N$个）基于$(\theta,I)$预测的状态$X_{pred,i}(\theta,I)$的误差正态分布概率乘积：
    \begin{equation}
    \pi(\theta,I) = \prod_{i=1}^{N}\mathcal{N}\bigl(X_{pred,i}(\theta,I) - X_{obsv,i},\sigma\bigr)
    \end{equation}
  ...
  \subsubsection*{(5)PPO方法核心思想与目标函数}
    ...
    \label{eq:ppo_clip}
    \end{equation}
    其中$\epsilon$为裁剪阈值（如0.1或0.2）。直观解释如下：
    \begin{itemize}
    \item 当$r_t(\theta)$偏离1（新旧策略差异过大）且会\emph{过度}提高目标时，裁剪项会限制增益，强制更新更保守。
    \item 通过$\min(\cdot)$结构，PPO在经验上倾向于避免“看似提升但可能导致崩溃”的过大更新。
    \end{itemize}

    PPO还通常加入价值函数损失与熵正则项构成整体目标：
    \begin{equation}
    L(\theta)=L^{\text{CLIP}}(\theta)-c_v\,\mathbb{E}_t\left[(V_\theta(s_t)-\hat{V}_t)^2\right]+c_e\,\mathbb{E}_t\left[\mathcal{H}(\pi_\theta(\cdot|s_t))\right],
    \label{eq:ppo_total}
    \end{equation}
    其中$V_\theta$为critic，$\hat{V}_t$为回报/TD目标，$\mathcal{H}$为策略熵，$c_v,c_e$为权重。熵项鼓励探索、降低过早收敛风险；价值损失为actor提供低方差优势估计支撑。PPO的工程实现通常采用“采样一批on-policy轨迹 $\rightarrow$ 多个epoch、小批量SGD更新”的训练范式。
  ...
  \subsubsection*{(6)优势函数估计与训练流程}
    ...
    \begin{enumerate}
    \item 使用当前策略$\pi_{\theta_{\text{old}}}$在环境中采样$N$步或若干条轨迹，得到$\{(s_t,a_t,r_t,s_{t+1})\}$；
    \item 用critic计算$V(s_t)$并构造$\hat{A}_t$与价值目标$\hat{V}_t$；
    \item 固定旧策略$\pi_{\theta_{\text{old}}}$，以小批量方式对目标$L(\theta)$进行$K$个epoch的梯度更新；
    \item 更新$\theta_{\text{old}}\leftarrow \theta$，进入下一轮采样。
    \end{enumerate}
...
\section{车端防侧翻避障应急轨迹规划}
  ...
  \subsection{半挂液罐车简化模型建立}
    ...
    \subsubsection*{(1)半挂车运动学模型}
      ...
      \begin{equation}
      \dot{\psi}_2 = \frac{v_{1x}}{L_2}\left( \frac{L_h}{L_1}\tan\delta \cos\gamma + \sin\gamma \right)
      \label{eq:150}
      \end{equation}

      其中定义铰接角$\gamma = \psi_1 - \psi_2$。
    ...
    \subsubsection*{(2)运动动力学耦合模型}
      ...
      \dot{Y}_{1r} = v_{1x} \sin\psi_1 \\
      \dot{\psi}_1 = \frac{v_{1x}}{L_1}\tan\delta \\
      \dot{\psi}_2 = K_v \frac{v_{1x}}{L_2}\left( \frac{L_h}{L_1}\tan\delta \cos\gamma + \sin\gamma \right), K_v = \left( 1 - \frac{m_2 v_{1x}^2 e}{L_1 L_2 k_3} \right)
      \end{cases}
      \label{eq:152}
      \end{equation}
      ...
      \begin{equation}
      \begin{cases}
      v_h^x = v_h \cos(\delta^{\prime} + \gamma) \\
      v_h^y = v_h \sin(\delta^{\prime} + \gamma)
      \end{cases}
      \label{eq:157}
      \end{equation}
      挂车等效后轴的速度沿着挂车纵向的分量也为$v_h^x$。而横向速度可以用该点的牵连速度分量$v_h^y$与相对运动速度分量表示，则可以表示挂车等效后轴的侧偏角为：
      \begin{equation}
      \alpha_3 \approx \tan\alpha_3 = \frac{v_h \sin(\delta^{\prime} + \gamma) - \dot{\psi}_2 L_2}{v_h \cos(\delta^{\prime} + \gamma)} = \tan(\delta^{\prime} + \gamma) - \frac{\dot{\psi}_2 L_2}{v_h \cos(\delta^{\prime} + \gamma)}
      \label{eq:158}
      \end{equation}
      接下来，对挂车的受力情况进行分析，可知挂车在第五轮受到牵引车的作用力、质心处由于圆周运动受到达朗贝尔力$F_{m2}$、后轴受到侧向力$F_y^3$与纵向力$F_x^3$，这些力合力为零。
      \begin{equation}
      F_y^3 \cos\theta_1 + F_x^3 \sin\theta_1 \approx k_3 \alpha_3 \cos\theta_1 = \frac{e}{L_2}F_{m2} = \frac{e m_2 v_2^2}{L_2 x} \approx \frac{e m_2 v_1^2}{L_2 L_1} \tan\delta \left( 1 - \frac{e}{L_1} \tan\delta \sin\gamma \right)
      \label{eq:159}
      \end{equation}
      最后一步的近似是基于：
      \begin{equation}
      \frac{v_2}{v_{1x}} = \frac{x}{R} \approx \frac{(R - e \sin\gamma)}{R} = \left( 1 - \frac{e}{L_1} \tan\delta \sin\gamma \right)
      \label{eq:160}
      \end{equation}
      基于式\eqref{eq:159}进一步化简：
      \begin{equation}
      \begin{aligned}
      &k_3 \tan(\delta^{\prime} + \gamma) - \frac{\dot{\psi}_2 L_2 k_3}{v_1 \cos(\delta^{\prime} + \gamma)} \approx \frac{e m_2 v_1^2}{L_2 L_1} \tan\delta \\
      \Longleftrightarrow~ &- \tan(\delta^{\prime} + \gamma) + \frac{\dot{\psi}_2 L_2}{v_1 \cos(\delta^{\prime} + \gamma)} = -\frac{e m_2 v_1^2}{L_2 L_1 k_3} \tan\delta \\
      \Longleftrightarrow~ &\frac{\dot{\psi}_2 L_2}{v_1} = -\frac{e m_2 v_1^2}{L_2 L_1 k_3} \tan\delta \cos(\delta^{\prime} + \gamma) + \sin(\delta^{\prime} + \gamma) \\
      \Longleftrightarrow~ &\dot{\psi}_2 = \frac{v_1}{L_2} \left( -\frac{e m_2 v_1^2}{L_2 L_1 k_3} \tan\delta \cos(\delta^{\prime} + \gamma) + \sin(\delta^{\prime} + \gamma) \right)
      \end{aligned}
      \label{eq:161}
      \end{equation}

      将其与式\eqref{eq:151}进行比较，$\dot{\psi}_2 = \frac{v_{1x}}{L_2}\left( \frac{L_h}{L_1}\tan\delta \cos\gamma + \sin\gamma \right)$，得到修正后的横摆角速度为：
      \begin{equation}
      \dot{\psi}_2 = K_v \frac{v_1}{L_2} \left( \frac{L_h}{L_1}\tan\delta \cos\gamma + \sin\gamma \right), K_v = \left( 1 - \frac{m_2 v_{1x}^2 e}{L_1 L_2 k_3} \right)
      \label{eq:162}
      \end{equation}
      ...
      \begin{equation}
      \begin{aligned}
      a_y &= \frac{v_{1x}^2}{m_1 + m_2} \left( \frac{m_1}{L_1} \tan\delta + \frac{m_2}{L_2} \cos^2 \gamma \sin\gamma \right) \\
      &= \frac{m_1 v_{1x}}{m_1 + m_2} \dot{\psi}_1 + \frac{m_2 v_{1x}^2}{L_2(m_1 + m_2)} \cos^2 \gamma \sin\gamma
      \end{aligned}
      \label{eq:163}
      \end{equation}
      ...
      设状态量$X = [x_1~ x_2~ x_3]^T$，输入量$u = [a_y~ \dot{\gamma}~ \gamma]^T$，输出是$y = \text{LTR}$则：
      \begin{equation}
      \begin{cases}
      \dot{X} = AX + Bu \\
      y = CX
      ...
      \dot{Y}_{1r} = v_{1x} \sin\psi_1 \\
      \dot{\psi}_1 = \frac{v_{1x}}{L_1}\tan\delta \\
      \dot{\gamma} = \dot{\psi}_1 - \frac{K_v}{L_2} \left( L_h \psi_1 \cos\gamma + v_{1x} \sin\gamma \right), K_v = \left( 1 - \frac{m_2 v_{1x}^2 e}{L_1 L_2 k_3} \right) \\
      \dot{\mathcal{L}} = \mathcal{L}_a + c_{11} v_{1x} \psi_1 + c_{21} v_{1x}^2 \cos^2 \gamma \sin\gamma + b_{31} \gamma, \quad c_{1i} = \frac{b_{1i} m_1}{m_1 + m_2}, c_{2i} = \frac{b_{1i} m_2}{L_2(m_1 + m_2)} \\
      \dot{\mathcal{L}}_a = \mathcal{L}_b + \left( c_{12} v_{1x} + b_{22} - \frac{b_{22} L_h}{L_2} \cos\gamma \right) \psi_1 + \left( c_{22} v_{1x}^2 \cos^2 \gamma - \frac{b_{22} v_{1x}}{L_2} \right) \sin\gamma + b_{32} \gamma \\
      \dot{\mathcal{L}}_b = a_1 \mathcal{L} + a_2 \mathcal{L}_a + a_3 \mathcal{L}_b + c_{13} v_{1x} \psi_1 + c_{23} v_{1x}^2 \cos^2 \gamma \sin\gamma
      \end{cases}
      \label{eq:166}
      \end{equation}
  ...
  \subsection{约束迭代LQR问题构造}
    ...
    利用上述约束，为保证侧翻稳定性，对车辆LTR以及铰接角$\gamma$都进行了CBF约束，其中为补偿线性LTR预测模型可能的误差，将近似LTR状态绝对值的CBF约束上限$\mathcal{L}_{lim}$设置为随时间指数衰减的形式:
    \begin{equation}
