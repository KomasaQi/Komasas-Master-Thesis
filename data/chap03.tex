...
本节介绍了一种基于图结构的流匹配扩散方法用于对周车进行长时域非确定预测，本节首先从图结构与图神经网络的介绍开始，随后展开了交通图特征表示方法及自回归图扩散交互式预测模型的构建方法。
  ...
  \subsection{自回归图扩散交互式预测模型}
    ...
    \subsubsection*{(1)训练数据结构}
      ...
      \noindent 其中，$F_{i,stat}$为静态特征，包含路网静态信息（如车道数、交通流方向等）；$F_{i,vehs}^{-T+k:k}$为历史加当前时刻周车动态特征（如速度、加速度、相对距离等）；$F_{i,ego}^{-T+k:k}$为历史与当前时刻自车占据情况，$\oplus$为向量拼接（Concatenete）操作，$\mathcal{V}^k$表示第$k$时刻的图结构路网节点集合。所有时刻的边特征集$\mathcal{E}$属于静态特征均保持不变。
