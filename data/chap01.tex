...
\section{国内外研究现状}
  ...
  \subsection{车云协同预测性决策与规划研究现状}
    ...
    \subsubsection{单车智能行为决策及其向云端的拓展}
      ...
      端到端方案则进一步通过通过学习从环境表征到行为/轨迹的可微分联合映射，在训练阶段将预测、决策与规划的耦合显式纳入优化目标，从而缓解传统“先预测后决策”带来的交互一致性问题。常见实现包括基于模仿学习的端到端轨迹生成、基于强化学习的交互策略学习，以及以扩散/生成模型为代表的多模态规划生成等。端到端方法中最早发展起来的两端式端到端融合预测--决策--规划三者，近年来涌现的一段式端到端则将感知--规划的闭环打通，其中VLA方案则将感知--控制全链路打通\cite{KomasaQi2026Appendix}。
      ...
      \begin{figure}
        ...
        \subcaptionbox{两段式端到端大尺度拓展\label{fig:pred_dec_int3}}
          {\includegraphics[width=0.385\linewidth]{fig/chap1/pred_dec_int3.png}}
