...
\begin{abstract}
液罐车在高等级公路运行过程中具有质心高、液体晃动显著和事故后果严重等特点，侧翻风险始终是其主动安全研究中的核心问题。现有研究在液体晃动建模、防侧翻规划与控制等方面已取得一定进展，但仍存在三个不足：缺乏面向车路云一体化系统的统一架构方法，难以利用广域环境信息开展长时域风险规避，以及面向液罐车车液耦合特性的快速建模、局部应急规划与高频稳定控制之间缺乏统一协同设计。针对上述问题，本文围绕车云协同液罐车防侧翻行驶规划与控制开展系统研究。

首先，面向车云协同液罐车防侧翻场景，提出了分场景So-MBSoSE体系建模方法，构建了CT2ROP场景任务体系架构。该方法建立了“战略—运行—服务—资源”四阶段递进的架构建模与闭环验证流程，为后续云端预测性决策、车端局部规划与高频控制提供了统一的系统边界、时间尺度约束和接口组织依据。

其次，围绕云端预测性决策与车端局部规划，提出了图扩散交互式预测、强化学习长时域决策、防侧翻换道轨迹生成以及车端防侧翻避障应急轨迹规划方法。云端侧实现了对周车长时域非确定性行为的预测，并输出速度与换道行为决策以及满足侧翻约束的参考轨迹；车端侧建立了简化模型与约束迭代LQR求解框架，将障碍避让、道路边界和防侧翻约束统一纳入局部轨迹重构过程，形成了面向局部突发风险的快速应急规划方法。
